...
def my_main(spark, my_dataset_dir):
    ...
    startStationDF = inputDF.groupBy(["start_station_name"]).agg({"start_station_name" : "count"})\
                                                            .withColumnRenamed("count(start_station_name)", "num_departure_trips")    
    startStationDF.persist()
    
    stopStationDF = inputDF.groupBy(["stop_station_name"]).agg({"stop_station_name" : "count"})\
                                                          .withColumnRenamed("count(stop_station_name)", "num_arrival_trips")
    stopStationDF.persist()
    
    solutionDF = startStationDF.join(stopStationDF,
                               startStationDF["start_station_name"] == stopStationDF["stop_station_name"],
                               "full_outer"
                               )
    
    solutionDF = solutionDF.withColumn("stop_station_name", pyspark.sql.functions.coalesce(solutionDF.stop_station_name, solutionDF.start_station_name))
    solutionDF = solutionDF.withColumn("start_station_name", pyspark.sql.functions.coalesce(solutionDF.start_station_name, solutionDF.stop_station_name))
    
    solutionDF = solutionDF.select(pyspark.sql.functions.col("stop_station_name").alias("station"),
                                   pyspark.sql.functions.col("num_departure_trips"),
                                   pyspark.sql.functions.col("num_arrival_trips"))
    
    solutionDF = solutionDF.na.fill(0)
    
    solutionDF = solutionDF.orderBy([solutionDF["station"].asc()])
    
    solutionDF = solutionDF.filter(solutionDF.station.isNotNull())
